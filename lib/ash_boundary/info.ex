defmodule AshBoundary.Info do
  ...
  def dirty_xrefs(domain), do: Extension.get_opt(domain, [:boundary], :dirty_xrefs, [])
  ...
  def check(domain), do: Extension.get_opt(domain, [:boundary], :check, nil)
  ...
  def type(domain), do: Extension.get_opt(domain, [:boundary], :type, nil)
