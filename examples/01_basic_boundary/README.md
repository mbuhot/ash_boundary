...
Examples 02 through 04 cover cross-domain `deps`, an exported-vs-internal
comparison, the calculation-based decoupling pattern, and a dedicated
violation-catching demo. See "Conventions for other examples (02-04)" below.
...
This example is the first of four sample projects. It sets the shape the other
examples follow.
...
- **The root repo's `.gitignore`** ignores `examples/*/_build/`,
  `examples/*/deps/`, and `examples/*/doc/` in one place. No example needs its
  own `.gitignore` with the same three lines.
...
- **Gate**: every example must independently pass, from within its own
  ...
  task-10 CI wiring loops over `examples/*/` and runs this gate for each
  example it finds. A project whose point is a caught violation still needs
  this gate to pass overall; the violation itself is demonstrated without
  making the shipped build red.
