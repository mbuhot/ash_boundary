...
| [`exports`](#boundary-exports){: #boundary-exports } | `list(module)` | `[]` | Public modules of this domain that are not resources. An `Ash.Type.Enum` named in an exported resource's attribute types is the usual case: outside code has to name it, so it belongs in the domain's API. Each module must be nested under the domain's namespace. A resource of this domain is rejected here. Resource exports come from `resources`, where a domain-level `define` makes a resource public. This option adds to that computed list rather than replacing it. |
| [`dirty_xrefs`](#boundary-dirty_xrefs){: #boundary-dirty_xrefs } | `list(module)` | `[]` | Passed through to `boundary`'s `dirty_xrefs` option, unchanged. |
| [`check`](#boundary-check){: #boundary-check } | `nil \| keyword` |  | Passed through to `boundary`'s `check` option, unchanged. Left unset, this domain gets whatever `boundary` itself defaults to, or whatever the consuming project sets as `boundary: [default: [check: ...]]` in `mix.exs`. AshBoundary does not change or add to it. |
| [`type`](#boundary-type){: #boundary-type } | `nil \| :strict \| :relaxed` |  | Passed through to `boundary`'s `type` option, unchanged. Left unset, this domain gets whatever `boundary` itself defaults to, or whatever the consuming project sets as `boundary: [default: [type: ...]]` in `mix.exs`. |
